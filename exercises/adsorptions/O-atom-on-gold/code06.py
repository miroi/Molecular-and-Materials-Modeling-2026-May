...
# Import CHGNet calculator
from chgnet.model import CHGNetCalculator

# --- 0. DEVICE SELECTION ---
device = "cuda" if torch.cuda.is_available() else "cpu"
print(f"CHGNet will run on: {device.upper()}")

# --- 1. ISOLATED ADSORBATE ---
# Place O atom in a small periodic box (to satisfy CHGNet requirements)
adsorbate = Atoms('O', cell=[10, 10, 10], pbc=True)
adsorbate.calc = CHGNetCalculator(device=device)
e_o = adsorbate.get_potential_energy()

# --- 2. CLEAN SLAB ---
slab = fcc111('Au', size=(6, 6, 3), vacuum=12.0)
slab.calc = CHGNetCalculator(device=device)
...
print(f"Calculated Binding Energy (CHGNet): {e_bind_calc:8.3f} eV")
print(f"Literature Value (DFT RPBE)   :     {lit_val:8.3f} eV")
